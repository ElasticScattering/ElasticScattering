...
Action ShowWindowParameters(ScatteringParameters &sp) {
    ...
    action.parameters_changed = false;
...
void MainLoop(ElasticScattering &es, ScatteringParameters& sp)
{
    ...
    Action action = ShowWindowParameters(sp);

    if ((sync_immediate && action.parameters_changed) || action.compute_requested) {
    ...
    auto textureId = es.GetTextureID();
