...
def box_intersects(xlow, ylow, L, xc, yc, rc, isEl):
    ...
    # Sadly the best way to write this.
    # Eliminate those triplets that start with 1e99.
    ints = ints[ints[:, 0] < 1e98, :]
...
def eval_lifetime(lifetimes, dimension, nr_phi, region, extends, alpha, iscoh):
    ...
    for xind in range(dimension):
        ...
        for yind in range(dimension):
            ...
            for pind in range(nr_phi):
                weightP = simpson_weight(pind, nr_phi)
                for quadrant in range(4):
                    pq_ind = pind + quadrant * nr_phi
...
def eval_sigma(lifetimes, tau, isEl, wc, mass, vf, caxis,
    ...
    for xind in range(dimension):
        ...
        for yind in range(dimension):
            weightY = simpson_weight(yind, dimension)
            for pind in range(nr_phi):
                # old code has dimension here erroniously!
                weightP = simpson_weight(pind, nr_phi)
                weight = weightX * weightY * weightP
                for quadrant in range(4):
                    pq_ind = pind + quadrant * nr_phi
    ...
    # These factors are stepsize as given by "interval / (nr - 1) / 6"
    # and for the x/y integrals we divide by the interval (=region) to get the
    # average rather than integral, leading to "1/(dimension-1)/6"
    #
    # Note that we use phirange/(nr_phi-1), both get a factor 4 from
    # quadrant which falls out. However, if you want the actual average
    # you would have to divide by 4*phirange.
    phirange = np.pi / 2 - 2 * alpha if iscoh else 2 * alpha
    integralxx *= phirange / ((nr_phi - 1) * (dimension - 1)**2 * 3**3)
    integralxy *= phirange / ((nr_phi - 1) * (dimension - 1)**2 * 3**3)

    print(integralxx / phirange / 4)

    # Note that in 2D, n=kf^2/(2pi^2 c)
    # So factor is really n*e^2t/[m(1+wc^2t^2)] and integral is just the
    # geometrical factor from the decay of coherence, which would be 1
    # in the absence of incoherence.
    kf = mass * vf / HBAR
    factor = E**2 * kf**2 * tau / (2 * np.pi**2 * mass * caxis)
    factor /= 1 + (wc * tau)**2
    sxx = integralxx * factor * 1e-8
    sxy = integralxy * factor * 1e-8
    return sxx, sxy
...
for i, T in enumerate(Tvals):
    ...
    drude = r[i, :, 0] * n * E**2 / mass * 1.001 / 1e8
    sxxc = r[i, :, 2]
    print(np.mean(r[i, :, 0]),':', np.mean(drude), 'vs', np.mean(sxxc))
