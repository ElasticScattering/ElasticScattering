...
#define GET_PARTICLE_INDEX(m_i, m_j) ((m_j * ss->values_per_row) + (m_i))
//#define GET_PARTICLE_LT_INDEX(m_i, m_j) ((m_j * ss->values_per_row) + (m_i * ss->values_per_particle))


#define GET_PHI(m_q, m_p) (ps->phi_start + m_q * HALF_PI + m_p * ps->phi_step_size)


#define GET_PHI(m_v) (ps->phi_start + floor(v / (double)ss->values_per_quadrant) * HALF_PI + (double)(v % ss->values_per_quadrant) * ps->phi_step_size)
